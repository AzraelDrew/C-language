...
void print(jobtable job[],int n){
    int i;
    printf("name in_well reun_time begin_run end_run turnover_time\n");
    for (i = 0; i < n;i++){
        printf("%s\t %0.1f\t %0.1f\t", job[i].name, job[i].in_well, job[i].run_time);
        if(job[i].begin_run ==0 && job[i].end_run ==0.0  && job[i].turnover_time==0.0){
            printf("                  \n");
        }
        else{
...
float response_ratio(jobtable job[],int n){
    ...
    for (i = 1; i < n; i++)
    {
		...
		if(job[i].in_well<=job[i-1].end_run){
			...
			while(j<n&&job[i-1].in_well<=job[i-1].end_run){
			
			ratio2=1+(job[i-1].end_run-job[j].in_well)*1.0/job[i].run_time;
